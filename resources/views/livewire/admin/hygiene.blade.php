                ...
                <div class="row">
                    ...
                    <div class="col-sm-6">
                        <ol class="breadcrumb float-sm-right">
                            ...
                            <li class="breadcrumb-item active">Produits hygiène et traitement</li>
                        </ol>
                    </div>
                </div>
            </div>
            ...
            <div class="col-12">
                <div class="card">
                    <div class="card-header bg-blue d-flex flex-wrap align-items-center">
                        <h3 class="card-title flex-grow-1 mb-2">Liste des produits de soins et hygiène</h3>
                        <div class="card-tools d-flex flex-wrap align-items-center">

                            <button class="btn text-white mb-2 mr-4 d-block" wire:click="showAddPrdtHygiene" style="background-color: rgb(59, 85, 202);">Créer un produit</button>
                    ...
                    <div class="card-body table-responsive p-0" style="height: 300px;">
                        <table class="table table-head-fixed text-nowrap table-striped">
                            ...
                            @forelse ($hygienes as $hygiene )
                                {{-- @if ($hyg->id_typeArticle == 3) --}}
                                    <tr>
                                        <td class="text-center">
                                            <img src="{{asset('storage/'.$hygiene->image_1)}}" width="60px" height="60px"/>
                                        </td>
                                        <td class="text-center">{{$hygiene->nom_produit}}</td>
                                        <td class="text-center">
                                            @if ((string)$hygiene->EstDisponible === "oui")
                                                <span class="badge badge-success">Oui</span>
                                            @else
                                                <span class="badge badge-danger">Non</span>
                                            @endif
                                        </td>
                                        <td class="text-center text-uppercase text-danger">{{$hygiene->prix_reel}}fcfa</td>
                                        <td class="text-center text-uppercase text-success">{{$hygiene->prix_promo}}fcfa</td>
                                        <td class="text-center">{{$hygiene->created_at->diffForHumans()}}</td>
                                        <td class="text-center">
                                            <button class="btn btn-primary" wire:click="showEditProduct({{$hygiene->id}})"><i class="far fa-edit fa-1x"></i></button>
                                            <button class="btn btn-success" wire:click="confirmDelete('{{$hygiene->nom_produit}}', {{$hygiene->id}})"><i class="far fa-trash-alt fa-1x"></i></button>
                                        </td>
                                    </tr>
                                {{-- @endif --}}
                                @empty
